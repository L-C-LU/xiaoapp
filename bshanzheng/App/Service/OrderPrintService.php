...
class OrderPrintService
{
     ...
     * @throws
     */
    public function queryStatus($order)
    {
        // 打印机设置
        $printerConfig = PrinterModel::create()->where('shop_id', $order['shop_id'])
            ->where('status', 1)
            ->get();
        ...
        // 执行打印请求
        return $PrinterDriver->queryStatus($order['print_order_id']);
    }
    ...
    public function printTicket($order, $shop, $isAuto)
    {
        ...
        var_dump('fff');
        // 执行打印请求
        $result =  $PrinterDriver->printTicket($content, $order['order_id']);

        if($printerConfig['type']==1){ //feie写订单号
            OrderListModel::create()->where('order_id', $order['order_id'])->update(['print_order_id' => ($result->data??'')]);
        }
        else if($printerConfig['type']==2){ //yilianyun写订单号
            OrderListModel::create()->where('order_id', $order['order_id'])->update(['print_order_id' => ($result['body']['id']??'')]);
        }
        return $result;
    }
    ...
    private function getPrintContent($shop, $order, $printerConfig)
    {
        ...
        $content = "<FS2><center>** 山政定食达**</center></FS2>";

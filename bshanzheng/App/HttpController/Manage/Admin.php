...
class Admin extends BaseController
{
    ...
    public function login(){
        ...
        if(!$user){
            return $this->apiBusinessFail("管理员名或密码错误!");
        }
        if($user['status']!=1){
    ...
    private function generateLoginInfo($user){
        $token = md5($user['user_id']. time(). ConstVar::$manageAppId);
        ...
        $userInfo = [
            ...
            "role_level" => $user["user_type"],
            "role_id" => $user["role_id"],
            'access' => [($user['user_type']>=50)?'admin':'user']
        ];
        ...
        $newToken = [
            'token' => $token,
            'user_id' => $user['user_id'],
            'app_id' => ConstVar::$manageAppId
        ];
        TokenModel::create($newToken)->save();
    ...
    public function logout()
    {
        ...
        TokenModel::create()->where('user_id', $this->getUserId())
            ->where('app_id', ConstVar::$manageAppId)
            ->destroy();
    ...
    public $rules_get = [
        'user_id|管理员Id' => 'require|number|max:16',
    ];
    ...
    public function list()
    {
        ...
        $this->setParam('user_type', [100,90,80]);
    ...
    public $rules_set_status = [
        'user_id|管理员Id' => 'require|number|max:16',
        'status|管理员状态' => 'require|number|max:1',
    ];
    ...
    public function setStatus(){
        ...
        if($userId == $this->getUserId()) return $this->apiBusinessFail("您不能变更您自己");

        if(!$user){
            return $this->apiBusinessFail("管理员信息未找到!");
        }
        ...
        if ($res) {
            ...
            return $this->apiBusinessFail("管理员状态更改失败");
        }
    }
    ...
    public $rules_update = [
        'user_id|管理员Id' => 'require|number|max:16',
        'contact_name|联系人名称' => 'require|max:50',
    ...
    public function addAdmin()
    {
        ...
        if (!$res) return $this->apiBusinessFail("管理员添加失败");
    ...
    public function delete(){
        ...
        if (!$res) {
            return $this->apiBusinessFail("管理员信息删除失败");
        }
